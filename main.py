...
with col1:
    ...
    if "Get_data" or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyCdhRrsb8aEBPNXUdNlbVjwFGgQtJRQdDk'
        youtube = build(api_service_name, api_version, developerKey=api_key)
        #channel_id = 'UC9xghV-TcBwGvK-aEMhpt5w'
        #channel_playlist_id='PLLa_h7BriLH1YRnhukwYetTOWgvJg2JBY&si=-9SEsCVUvcuyh0ah'
...
# Extract required information from the channel_data
        ...
        #channel_playlist_id='PLLa_h7BriLH1jmdJ7PwTrz8yYWe0-WUCf&si=Is_JipapOU4M_GT3'
        ...
        # Access the database (replace 'your_database' with the actual name of your database)
        db = client.youtube_db

        # Access the collection within the database (replace 'youtube_data' with the actual name of your collection)
    ...
    if 'Migrate' or st.session_state_migrate_sql:
        ...
        playlist_df.to_sql('playlist', engine, if_exists='append', index=False,
                           DType={"Channel_Id": sqlalchemy.types.VARCHAR(225),
                                  "Playlist_Id": sqlalchemy.types.VARCHAR(225), })
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='Prashanthi@19', db='youtube_db')
